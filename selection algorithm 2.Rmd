...
```{r}
fixed <- c(1,4,6,7,9,30,10,12,14,15,45)
...
paste0("Testing ",nrow(rep.6c)*nrow(rep.4c)," combinations")
```
...
while (n <= length(index.com)) {
  ...
  n <- n + 1
  if (count == 5) {
    time.elapsed <- difftime(Sys.time(), time.start, units = "mins")
    n.left <- length(index.com) - n
    time.left <- time.elapsed/count * n.left
    if (time.left > 15) n <- length(index.com)
  ...
  if (count != 5) {
    ...
    if (count >= 10) if (round(count/20) == count/20) print(round(time.left))
  }
}
```
