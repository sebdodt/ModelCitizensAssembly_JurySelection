...
if (nrow(young.white.male) < cat5) {
  ...
  if (extra.men[3] == 2) cat6 <- cat6 + 1
  if (extra.men[3] == 3) cat7 <- cat7 + 1
  if (extra.men[3] == 4) cat8 <- cat8 + 1
}
...
Combinations new
```{r}
comb.young.white.female <- combinations(n = nrow(young.white.female), r = cat1)
comb.young.non.white.female <- combinations(n = nrow(young.non.white.female), r = cat3)
comb.young.white.male <- combinations(n = nrow(young.white.male), r = cat5)
comb.young.non.white.male <- combinations(n = nrow(young.non.white.male), r = cat7)
comb.old.white.female <- combinations(n = nrow(old.white.female), r = cat2)
comb.old.non.white.female <- combinations(n = nrow(old.non.white.female), r = cat4)
comb.old.white.male <- combinations(n = nrow(old.white.male), r = cat6)
comb.old.non.white.male <- combinations(n = nrow(old.non.white.male), r = cat8)

#number of combinations
ncombinations <- nrow(comb.young.white.female) * nrow(comb.young.non.white.female) * 
  nrow(comb.young.white.male) * nrow(comb.young.non.white.male) *
  nrow(comb.old.white.female) * nrow(comb.old.non.white.female) * 
  nrow(comb.old.white.male) * nrow(comb.old.non.white.male) 

#combinations matrix
rep.1 <- comb.white.female[rep(seq_len(nrow(comb.white.female)), each = nrow(comb.non.white.female)), ]
rep.2 <- comb.white.male[rep(seq_len(nrow(comb.white.male)), each = nrow(comb.non.white.male)), ]
rep.3 
```
